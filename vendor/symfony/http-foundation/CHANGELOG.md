 ...
 * Improved error-handling of `\Symfony\Component\HttpFoundation\Session\Storage\Handler\PdoSessionHandler`
   to ensure the supplied PDO handler throws Exception on error (as the class expects). Added related test cases
   to verify that Exception are properly thrown when the PDO queries fail.
